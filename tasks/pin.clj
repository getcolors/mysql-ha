(ns pin
  ...
  mysql-ha's launcher pins only mysql-ha. `green` comes transitively from
  mysql-ha's deps.edn, so there is one site here."
  (:require
...
(def ^:private site
  "Where a copied payload records which mysql-ha commit to resolve.
  ...
  {:path "skills/package-mysql-ha-green/green"
   :rx #"\(def \^:private mysql-ha-sha (nil|\"[0-9a-f]{40}\")\)"})
  ...
  (let [file (io/file (:path site))]
    (if-not (.exists file)
      ...
      (let [text (slurp file)
        ...
        (if-not current
          {:green/exit 2
           :green/err (str "could not locate mysql-ha-sha in " (:path site))}
          (let [[head err] (repo-head "." "mysql-ha")]
            (cond
