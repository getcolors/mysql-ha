---
name: package-mysql-ha-blue
description: Build and operate a three-member MySQL Group Replication cluster on DigitalOcean with Blue — automatic failover onto a reserved IP, daily snapshots and continuous binary-log archiving to Cloudflare R2, and a scheduled verified restore.
license: MIT
---
...
```sh
./blue build              # render the work directory only
./blue create --dry-run   # walk the graph, contact nothing
./blue create             # converge, then snapshot, verify a restore, and assert
./blue health             # read-only assertions against the live cluster
./blue delete             # guarded teardown
```

A real lifecycle run needs uv, OpenTofu, Ansible and SSH; the shipped
